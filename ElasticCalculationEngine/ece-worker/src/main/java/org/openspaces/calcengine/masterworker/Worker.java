...
public class Worker implements ClusterInfoAware, InitializingBean {
	...
	public Result execute(Request request) {
		...
		try {
			HashMap<String, Double> resultData = CalculateNPVUtil.execute(
					gigaspaceLocalCache, gigaspace, request.getTradeIds(),
					workerID, request.getRate());
	...
	public void afterPropertiesSet() throws Exception {
		IJSpace localSpaceProxy = new LocalCacheSpaceConfigurer(
				gigaspace.getSpace()).localCache();
		gigaspaceLocalCache = new GigaSpaceConfigurer(localSpaceProxy)
				.gigaSpace();
	}
