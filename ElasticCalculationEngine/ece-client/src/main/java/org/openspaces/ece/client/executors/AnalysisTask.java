...
public class AnalysisTask implements Task<HashMap<String, Double>> {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -8499929876575703786L;
	@TaskGigaSpace
	...
	public HashMap<String, Double> execute() throws Exception {
		int partitionID = Integer.valueOf(
				gigaspace.getSpace().getURL()
						.getProperty(SpaceURL.CLUSTER_MEMBER_ID)).intValue() - 1;
		System.out.println("Time: " + System.currentTimeMillis()
				+ " Execute on Partition ID: " + partitionID + " Routing ID:"
				+ routing);
		return CalculateNPVUtil.execute(gigaspace, gigaspace, tradeIds,
				partitionID, rate);
	}
